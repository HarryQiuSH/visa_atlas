...
def build_sidebar_filters(source_info: DataSourceInfo, bounds: FilterBounds) -> FilterState:
    ...
    with st.sidebar:
        ...
        # Dynamic employer search with multi-select
        if get_canonical_column(source_info, "employer_name") is None:
            st.text_input("Search employers", disabled=True, key="filter_employer_search")
            employer_names: tuple[str, ...] = ()
        else:
            employer_search = st.text_input(
                "Search employers",
                key="filter_employer_search",
                placeholder="Type to search...",
                help="Type to find employers, then select multiple from results"
            )
            
            if employer_search.strip():
                employer_matches = get_matching_values(source_info, "employer_name", employer_search, limit=20)
            else:
                employer_matches = []
            
            selected_employers = st.multiselect(
                "Select employer(s)",
                options=employer_matches,
                key="filter_employer",
                help="You can select multiple employers"
            )
            employer_names = tuple(selected_employers)
        
        # Dynamic job title search with multi-select
        if get_canonical_column(source_info, "job_title") is None:
            st.text_input("Search job titles", disabled=True, key="filter_job_title_search")
            job_titles: tuple[str, ...] = ()
        else:
            job_title_search = st.text_input(
                "Search job titles",
                key="filter_job_title_search",
                placeholder="Type to search...",
                help="Type to find job titles, then select multiple from results"
            )
            
            if job_title_search.strip():
                job_title_matches = get_matching_values(source_info, "job_title", job_title_search, limit=20)
            else:
                job_title_matches = []
            
            selected_job_titles = st.multiselect(
                "Select job title(s)",
                options=job_title_matches,
                key="filter_job_title",
                help="You can select multiple job titles"
            )
            job_titles = tuple(selected_job_titles)
