...
def build_where_clause(source_info: DataSourceInfo, filters: FilterState) -> tuple[str, list[object]]:
    ...
    def add_in_clause(canonical_name: str, values: tuple[str, ...]) -> None:
        expr = _get_field_expr(source_info, canonical_name)
    ...
    # Global filter: Only show data from 2022 onwards (including 2022)
    submit_date_expr = _get_field_expr(source_info, "case_submit_date")
    if submit_date_expr:
        conditions.append(f"EXTRACT(YEAR FROM {submit_date_expr}) >= 2022")
    
    # Handle multiple employers
    if filters.employer_name:
        add_in_clause("employer_name", filters.employer_name)
    
    # Handle multiple job titles
    if filters.job_title:
        add_in_clause("job_title", filters.job_title)
...
def get_time_trend(source_info: DataSourceInfo, filters: FilterState) -> pd.DataFrame:
    ...
    status_expr = _get_field_expr(source_info, "case_status")
    if not date_expr:
        return pd.DataFrame()
    where_sql, params = build_where_clause(source_info, filters)
    ...
    if status_expr:
        ...
        sql = f"""
            SELECT
                ...
                UPPER(CAST({status_expr} AS VARCHAR)) AS status,
                COUNT(*) AS case_count
...
@st.cache_data(show_spinner=False)
def get_status_by_fiscal_year(source_info: DataSourceInfo, filters: FilterState) -> pd.DataFrame:
    """Get approval, denial, withdrawal counts by fiscal year."""
    date_expr = _get_field_expr(source_info, "case_submit_date")
    status_expr = _get_field_expr(source_info, "case_status")
    
    if not date_expr or not status_expr:
        return pd.DataFrame()
    
    where_sql, params = build_where_clause(source_info, filters)
    extra_filter = f"{where_sql} AND {date_expr} IS NOT NULL AND {status_expr} IS NOT NULL" if where_sql else f" WHERE {date_expr} IS NOT NULL AND {status_expr} IS NOT NULL"
    
    # Calculate fiscal year (Oct 1 - Sept 30)
    sql = f"""
        SELECT
            CASE 
                WHEN EXTRACT(MONTH FROM {date_expr}) >= 10 THEN EXTRACT(YEAR FROM {date_expr}) + 1
                ELSE EXTRACT(YEAR FROM {date_expr})
            END AS fiscal_year,
            UPPER(CAST({status_expr} AS VARCHAR)) AS status,
            COUNT(*) AS count
        FROM ({source_info.table_sql}) AS src
...
@st.cache_data(show_spinner=False)
def get_approval_denial_rates(source_info: DataSourceInfo, filters: FilterState) -> pd.DataFrame:
    """Calculate approval and denial rates by fiscal year."""
    df = get_status_by_fiscal_year(source_info, filters)
    
    if df.empty:
        return pd.DataFrame()
    
    # Pivot to get counts by status
    pivot_df = df.pivot(index='fiscal_year', columns='status', values='count').fillna(0)
    
    # Calculate rates
    result_df = pd.DataFrame()
    result_df['fiscal_year'] = pivot_df.index
    
    # Identify approved statuses (certified + certified-withdrawn)
    approved_cols = [col for col in pivot_df.columns if 'CERTIFIED' in col]
    denied_cols = [col for col in pivot_df.columns if 'DENIED' in col]
    
    if approved_cols:
        result_df['approved'] = pivot_df[approved_cols].sum(axis=1)
    else:
        result_df['approved'] = 0
        
    if denied_cols:
        result_df['denied'] = pivot_df[denied_cols].sum(axis=1)
    else:
        result_df['denied'] = 0
    
    # Total is ALL submissions (including withdrawn, etc.)
    result_df['total'] = pivot_df.sum(axis=1)
    
    # Filter out rows with no data
    result_df = result_df[result_df['total'] > 0].copy()
    
    if result_df.empty:
        return pd.DataFrame()
    
    # Approval rate = (certified + certified-withdrawn) / all submissions
    result_df['approval_rate'] = (result_df['approved'] / result_df['total'] * 100).round(2)
    
    # Denial rate = denied / all submissions
    result_df['denial_rate'] = (result_df['denied'] / result_df['total'] * 100).round(2)
