...
def build_trend_chart(df: pd.DataFrame) -> go.Figure | None:
    """Create a monthly filings trend chart with status breakdown."""
    if df.empty:
        return None
    
    # Check if we have status column
    if "status" in df.columns:
        # Multi-line chart for different statuses
        figure = px.line(
            df,
            x="filing_month",
            y="case_count",
            color="status",
            markers=True,
            title="Filings trend over time by status",
        )
    else:
