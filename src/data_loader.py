...
class DataSourceInfo:
    ...
    skipped_parquet_files: tuple[str, ...]
    table_sql: str
...
def build_r2_source_paths(base_url: str, parquet_files: tuple[str, ...]) -> tuple[str, ...]:
    """Return full HTTPS parquet paths for a Cloudflare R2 base URL and file names."""
    normalized_base_url = base_url.strip().rstrip("/")
    return tuple(f"{normalized_base_url}/{file_name.strip().lstrip('/')}" for file_name in parquet_files)
...
def get_r2_parquet_file_names(data_dir: Path) -> tuple[str, ...]:
    ...
    return tuple(path.name for path in discover_parquet_files(data_dir))
...
def discover_data_source(
    ...
    if source_kind == "cloudflare_r2":
        ...
        if not base_url:
            raise DataSourceError("Set `R2_BASE_URL` in `.env` for the Cloudflare R2 source.")

        parquet_file_names = get_r2_parquet_file_names(data_dir)
        if not parquet_file_names:
            raise DataSourceError(
                "No parquet file list is available for the Cloudflare R2 source. Keep local parquet files available so the app "
    ...
    try:
        ...
        if source_kind == "cloudflare_r2":
            raise DataSourceError(
                "Unable to read parquet files from Cloudflare R2. Confirm the base URL, parquet file names, network access, "
                "and that the objects are publicly readable.\n\n"
                f"First URL attempted: {reachable_parquet_files[0]}\n"
                f"DuckDB error: {exc}"
            ) from exc
    ...
    return DataSourceInfo(
        ...
        parquet_files=reachable_parquet_files,
        skipped_parquet_files=skipped_parquet_files,
        table_sql=table_sql,
